...
////////////////////////////////////////////////////////////////////////////////////////////////////
*/
SR_PRIV int ezusb_fx10_ram_write (libusb_device_handle *hdl,
        unsigned char *buf,
        unsigned int   ramAddress,
        int            len)
{
	...
	return SR_OK; // initially FX10 Flash will be programmed externally
    ...
    while (len > 0) {
        ...
        if (r != size) {
            sr_err ("Error: Vendor write to FX10 RAM failed\n");
            return SR_ERR;
        }
        ramAddress += size;
        index      += size;
...
SR_PRIV int ezusb_install_firmware_fx10(struct sr_context *ctx,
				   libusb_device_handle *hdl,
				   const char *name)
{
	unsigned char *firmware;
	...
	/* Max size of FX10 fw file is 512 KB  = 1 << 19*/
	firmware = sr_resource_load(ctx, SR_RESOURCE_FIRMWARE,
			name, &filesize, 1 << 19);
	if (!firmware)
		return SR_ERR;

	sr_info("Uploading FX10 firmware '%s'.", name);
    ...
    while (index < filesize) {
        ...
        if (length != 0) {
            for (i = 0; i < length; i++)
                checksum += data_p[2 + i];
            r = ezusb_fx10_ram_write (hdl, firmware + index + 8, address, length * 4);
            if (r != 0) {
                sr_err ("Error: Failed to download data to FX10 RAM\n");
                free (firmware);
                return SR_ERR;
            }
        } else {
...
SR_PRIV int ezusb_upload_firmware_fx10(struct sr_context *ctx, libusb_device *dev,
				  int configuration, const char *name)
{
	struct libusb_device_handle *hdl;
	int ret;
	...
	sr_info("uploading firmware to FX10 device on %d.%d",
		libusb_get_bus_number(dev), libusb_get_device_address(dev));

	if ((ret = libusb_open(dev, &hdl)) < 0) {
		sr_err("failed to open FX10 device: %s.", libusb_error_name(ret));
		return SR_ERR;
	}
 ...
 */
	...
	if (ezusb_install_firmware_fx10(ctx, hdl, name) < 0)
		return SR_ERR;
