...
static int command_start_acquisition(const struct sr_dev_inst *sdi)
{
	...
	cmd.sampling_factor = (FX10_PIB_CLOCK)/(samplerate);
...
SR_PRIV int cypress_fx10_dev_open(struct sr_dev_inst *sdi, struct sr_dev_driver *di)
{
	...
	for (i = 0; i < device_count; i++) {
		 ...
		 */
		if (vi.major != FX10_REQUIRED_VERSION_MAJOR) {
			sr_err("Expected firmware version %d.x, "
			       "got %d.%d.", FX10_REQUIRED_VERSION_MAJOR,
			       vi.major, vi.minor);
			break;
		}
		...
		sr_info("Detected REVID, it's a Cypress FX10!\n");
