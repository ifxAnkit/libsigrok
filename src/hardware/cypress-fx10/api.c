...
static GSList *scan(struct sr_dev_driver *di, GSList *options)
{
	...
	const struct cypress_fx10_profile *prof;
	...
	/* Find all cypress_fx10 compatible devices and upload firmware to them. */
	devices = NULL;
	...
	for (i = 0; devlist[i]; i++) {
		...
		if (has_firmware) {
			...
			sr_dbg("Found an Cypress FX10 device.");
...
static int dev_open(struct sr_dev_inst *sdi)
{
	 ...
	 * milliseconds for the fx10 to renumerate.
	 */
	ret = SR_ERR;
	
	if (devc->fw_updated > 0) {
		sr_info("Waiting for device to reset.");
		/* Takes >= 300ms for the fx10 to be gone from the USB bus. */
		g_usleep(300 * 1000);
		timediff_ms = 0;
		while (timediff_ms < MAX_RENUM_DELAY_MS) {
			if ((ret = cypress_fx10_dev_open(sdi, di)) == SR_OK)
				break;
			g_usleep(100 * 1000);
		...
		ret = cypress_fx10_dev_open(sdi, di);
	}
...
static struct sr_dev_driver cypress_fx10_driver_info = {
	.name = "cypress-fx10",
	.longname = "cypress-fx10 (generic driver for FX10 based LAs)",
	.api_version = 1,
	.init = std_init,
	.cleanup = std_cleanup,
	.scan = scan,
	...
	.dev_open = dev_open,
	.dev_close = dev_close,
	.dev_acquisition_start = cypress_fx10_start_acquisition,
	.dev_acquisition_stop = cypress_fx10_acquisition_stop,
	.context = NULL,
};
SR_REGISTER_DEV_DRIVER(cypress_fx10_driver_info);
