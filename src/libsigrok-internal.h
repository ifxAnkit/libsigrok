...
SR_PRIV int ezusb_upload_firmware_fx10(struct sr_context *ctx, libusb_device *dev,
				  int configuration, const char *name);
SR_PRIV int ezusb_install_firmware_fx10(struct sr_context *ctx,libusb_device_handle *hdl,
					const char *name);
SR_PRIV int ezusb_fx10_ram_write (libusb_device_handle *hdl, unsigned char *buf, 
					unsigned int ramAddress, int len);					
#endif
